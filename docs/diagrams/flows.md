...
- [WhatsApp Expense Capture](whatsapp-expense-capture.mmd): Upstream sender allowlisting, defensive registered-user checks, deterministic parsing, clarification, and expense persistence.
...
The diagrams describe the MVP from user identity through daily financial tracking. Users authenticate with WhatsApp OTP, store access and refresh tokens, and every frontend request is scoped by the tenant id inside the JWT. Expired access tokens are refreshed once before the original request is retried. Expenses can enter through WhatsApp only for registered/test-approved senders or through the manual Angular form; incomes, categories, budgets, profile data, and report preferences are managed from the dashboard. Report data is read by tenant and period, with totals kept separated by currency until exchange-rate conversion is introduced.
