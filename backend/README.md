...
- `MESSAGE_INTERPRETER_PROVIDER`: `deterministic` or `openai-compatible`.
- `MESSAGE_INTERPRETER_API_KEY`: API key for the selected LLM provider. Leave empty to fall back to deterministic parsing.
- `MESSAGE_INTERPRETER_BASE_URL`: OpenAI-compatible chat completions base URL. Defaults to DeepSeek's API base URL.
- `MESSAGE_INTERPRETER_MODEL`: model name. For DeepSeek V3-style usage, set the provider model configured for your DeepSeek account, for example `DeepSeek-V3-0324` when that is the exposed model id for your provider.
...
Inbound text is interpreted through the `MessageInterpreterPort`. The default deterministic interpreter supports basic expense, income, report, and budget-status intents. Set `MESSAGE_INTERPRETER_PROVIDER=openai-compatible` with an API key, base URL, and model to use a DeepSeek/OpenAI-compatible chat completions provider. The provider only proposes structured JSON; the backend still validates required fields, tenant scope, categories, and persistence rules before saving or replying.
